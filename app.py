...
def main():
    ...
    if st.button('Predict'):
        ...
        # Display comparison with average, max, and min prices
        ...
        if len(st.session_state['predictions']) >= 5:
            st.session_state['predictions'].pop(0)  # Remove the oldest prediction
        ...
        ax.set_xticklabels([f'Attempt {i+1}' for i in range(len(st.session_state['predictions']))])
        st.pyplot(fig)
