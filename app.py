...
def user_input_features(neighbourhood_group=None, room_type=None):
    ...
    minimum_nights = st.sidebar.number_input('Minimum Nights', min_value=1, step=1)
    number_of_reviews = st.sidebar.number_input('Number of Reviews', min_value=0, step=1)
    reviews_per_month = st.sidebar.number_input('Reviews per Month', min_value=0.0, step=0.1)
    calculated_host_listings_count = st.sidebar.number_input('Calculated Host Listings Count', min_value=0, step=1)
    availability_365 = st.sidebar.number_input('Availability 365', min_value=0, max_value=365, step=1)
...
def main():
    ...
    if st.button('Predict'):
        ...
        for group in neighbourhood_groups:
            temp_df = user_input_features(neighbourhood_group=group, room_type='Entire home/apt')  
            prediction = model.predict(temp_df)
            prices[group] = prediction[0]
        
		
		fig, ax = plt.subplots()
        ax.bar(prices.keys(), prices.values(), color=['blue', 'green', 'red', 'purple', 'orange'])
        ax.set_ylabel('Price')
        ax.set_title('Price Comparison with neighbourhood_group')